...
function showQR(id){
 ...
 $("#modalContent").innerHTML=`<h3>${esc(s.name)} のスタッフQR</h3>
 <div class="qrbox"><div id="qrTarget"></div></div>
 <p style="text-align:center;color:var(--muted)">ID: ${esc(s.id)}</p>
 <p style="text-align:center;display:flex;gap:8px;justify-content:center">
   <button class="primary" id="downloadQR">PNG保存</button>
   <button class="ghost" id="printQR">印刷</button>
 </p>`;
 ...
 const draw=()=>{
   if(typeof QRCode==="undefined"){
     target.innerHTML=`<div style="padding:25px;color:#222;background:#fff;border-radius:8px">
       <strong>QRライブラリを読み込めませんでした</strong><br>
       <small>ページを再読み込みしてください。</small></div>`;
     return;
   }
   QRCode.toCanvas(s.id,{width:260,margin:2,errorCorrectionLevel:"M"},(err,canvas)=>{
     if(err){
       console.error(err);
       target.innerHTML=`<p style="color:#d33">QRコードの生成に失敗しました。</p>`;
       return;
     }
     ...
     $("#printQR").onclick=()=>{
       ...
       if(!w){alert("ポップアップがブロックされています。許可してからもう一度お試しください。");return}
       w.document.write(`<html><head><title>${esc(s.name)} QR</title></head>
       <body style="text-align:center;font-family:sans-serif;padding:40px">
       <h2>${esc(s.name)}</h2><img src="${data}" style="width:300px;height:300px"><p>${esc(s.id)}</p>
       </body></html>`);
       w.document.close();
